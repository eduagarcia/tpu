...
def _convert_to_example(filename, image_buffer, label, synset, sex, height, width):
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/class/sex': _int64_feature(sex),
...
def _process_image_files_batch(coder, output_file, filenames, synsets, labels):
  ...
  for filename, synset in zip(filenames, synsets):
    ...
    label, sex = labels[synset]
    example = _convert_to_example(filename, image_buffer, label,
                                  synset, sex, height, width)
...
def convert_to_tf_records(raw_data_dir):
  ...
  files = tf.gfile.Glob(
      os.path.join(raw_data_dir, 'images', '*.png'))
  ...
  split = int(len(files) * VALIDATION_SPLIT)
  training_files = files[split:]
  training_synsets = synsets[split:]
  validation_files = files[:split]
  validation_synsets = synsets[split:]
  
  #bits = tf.read_file(os.path.join(raw_data_dir, 'train.csv'))
  #string_data = StringIO(bits.numpy().decode('utf-8'))
  boneage_df = pd.read_csv(os.path.join(raw_data_dir, 'train.csv')) 
  # Create unique ids for all synsets
  labels = {row['fileName']: (int(row['boneage']), int(row['male'])) for i, row in boneage_df.iterrows()}
